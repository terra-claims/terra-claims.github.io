...
let osm = Leaflet.tileLayer(osmUrl, Leaflet.tile_layer_options(~attribution=osmAttrib, ()) );	
...
let make = (_children) => {
  ...
  reducer: (action, state) => {
    switch (action) {
    | SetMap(map) => ReasonReact.UpdateWithSideEffects({...state, map: Some(map)}, self => {
    map |. Leaflet.Map.setView(self.state.center, self.state.zoom) |. Leaflet.Map.addLayer(osm) |. ignore })
    | Move(lat,lon) => ReasonReact.SideEffects(self => moveTo({...self.state, center: (lat,lon)}))
    | UpdateCenter(lat, lon) => ReasonReact.Update({...state, center: (lat, lon)})
    | Zoom(zoom) => ReasonReact.SideEffects( self => zoomTo({...self.state, zoom}))
  ...
  didMount: self => {
    ...
    let updateMove = () => {
      ...
      | None => ()
      | Some(pos) => {
        self.send(UpdateCenter(Leaflet.LatLng.lat(pos), Leaflet.LatLng.lng(pos)));
        ();
      }
      };
    };
